            ...
            if next_request is None:
                if queue_of_requests_to_retry.empty() is False:
                    ...
                    try:
                        ...
                        task_id, others, request_json = content["task_id"], content[
                            "others"], content["request"]
    ...
    async def call_API(
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                append_to_jsonl([{"task_id": self.task_id}, self.request_json, self.result, {"others":self.others}], save_filepath)
                status_tracker.num_tasks_in_progress -= 1
                status_tracker.num_tasks_failed += 1
        else:
            append_to_jsonl([{"task_id": self.task_id}, self.request_json, response, {"others":self.others}], save_filepath)
            status_tracker.num_tasks_in_progress -= 1
